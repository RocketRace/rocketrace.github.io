...
addEventListener("load", _ => {
    const FAR_CANVAS = document.getElementById("furthest-neighbor");
    const NEAR_CANVAS = document.getElementById("nearest-neighbor");
    const FAR_CTX = FAR_CANVAS.getContext("2d");
    const NEAR_CTX = NEAR_CANVAS.getContext("2d");
    const antiround = (z) => z < 0.5 ? Math.ceil(z) : Math.floor(z);
    const interpolatePointFar = (x, y, scale) => {
        const sx = antiround(x / scale);
        const sy = antiround(y / scale);
        return FAR_CTX.getImageData(sx, sy, 1, 1).data;
    };
    const interpolatePointNear = (x, y, scale) => {
    ...
    const WIDTH = 200;
    const HEIGHT = 200;
    const interpolateImage = (scale) => {
        FAR_CTX.reset();
        NEAR_CTX.reset();
        const SOURCE_IMAGE = document.getElementById("furthest-neighbor-image");
    ...
    const SLIDER = document.getElementById("furthest-neighbor-scale");
    let current_scale = 1;
